...
func (u userDo) FindByUUID(uuid string) (result *model.User, err error) {
	params := make(map[string]interface{}, 0)

	var generateSQL strings.Builder
	params["uuid"] = uuid
	generateSQL.WriteString("select * from users where uuid=@uuid ")
...
func (u userDo) InsertValue(uuid string, age int, name string, version int) (err error) {
	params := make(map[string]interface{}, 0)
	...
	generateSQL.WriteString("insert into users (uuid, name ,age, version) values (@uuid,@name,@age,@version) ")
...
func (u userDo) UpdateVersion(uuid string, age int, name string, version int) (err error) {
	params := make(map[string]interface{}, 0)
	...
	params["version"] = version
	params["uuid"] = uuid
	generateSQL.WriteString("update users set name = @name,age = @age, version = @version where uuid = @uuid ")
...
func (u userDo) DeleteValue(uuid string) (err error) {
	params := make(map[string]interface{}, 0)

	var generateSQL strings.Builder
	params["uuid"] = uuid
	generateSQL.WriteString("delete from users where uuid = @uuid ")
...
func (u userDo) FindMaxVersion() (result int, err error) {
	var generateSQL strings.Builder
	generateSQL.WriteString("select sum(version) from users where version = (select version from users group by version ORDER BY version desc limit 1) ")
